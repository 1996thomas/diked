...
export default function Experience({ rotationY, scrollPosition, htmlWidth, pagesLength }) {
  ...
  useEffect(() => {
    if (htmlWidth > 0 && pagesLength > 0) {
      ...
      const sectionRelativePosition = (scrollPosition % sectionWidth) / sectionWidth;
    ...
    <>
      ...
      <group ref={modelRef}>
        {models.map((model, index) => (
          <Model key={index} url={model.url} color={model.color} opacity={opacities[index]} />
        ))}
      </group>
    </>
  );
